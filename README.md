...
## Running the Speedrun on Lambda.ai

For users who want to run the speedrun on Lambda.ai cloud infrastructure:

1. **Create a Lambda.ai account** and set up billing.

2. **Launch an 8xH100 instance**: `8x H100 SXM5 80GB`. At the time of writing, this costs ~$24/hour. As part of this process you will also create a filesystem. You will also create an SSH key pair.

3. **Connect to your instance** via SSH:
   ```bash
   chmod 400 /path/to/your/lambda-key.pem
   ssh -i /path/to/your/lambda-key.pem ubuntu@<your-instance-ip>
   ```
   Replace `/path/to/your/lambda-key.pem` with the actual path to your downloaded `.pem` file and `<your-instance-ip>` with the IP address shown in your Lambda instance details.

4. **Set up the environment and start the speedrun**:
   ```bash
   sudo apt update && sudo apt install -y git python3-pip htop nvtop && git clone https://github.com/KellerJordan/modded-nanogpt.git && cd modded-nanogpt && pip install -r requirements.txt && pip install --pre torch --index-url https://download.pytorch.org/whl/nightly/cu126 --upgrade && python data/cached_fineweb10B.py 8 && echo "✅ Setup complete!"
   ```
...
6. **Copy results to your local machine** (run this on your local machine):
   ```bash
   scp -i /path/to/your/lambda-key.pem ubuntu@<your-instance-ip>:/home/ubuntu/modded-nanogpt/<results-file>.txt ~/Downloads/
   ```
   Replace `<results-file>` with the actual filename generated by the training script.

7. **Important**: Immediately after copying the results file, terminate your Lambda.ai instance to avoid incurring additional charges. You can do this from the Lambda.ai console. Delete also the filesystem you created, as it will not be needed anymore. The total cost at the time of writing should be ~ $5 at $24/hr.

8. **Analyze results locally**: You can use the `analyze_results.py` script in `verification_results/` to visualize your results.
